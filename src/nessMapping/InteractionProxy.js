      ...
      if (olInteraction) {
        ...
        if (sourceLayer) {
          olInteraction.set(
            ...
            this.parent.setVectorSource(sourceLayer),
            true
          );
        }
        if (vLayer) {
          olInteraction.set(
            ...
            this.parent.setGraphicLayer(vLayer),
            true
          );
        }
...
const _toOLInteraction = (ni) => {
  let olInteraction,
    sourceLayer,
    vLayer = null;
  let interactionConfig = ni.config.interactionConfig
    ? ni.config.interactionConfig
    : null;
  switch (ni.config.Type) {
    case "Draw":
      const { Interaction, vectorSource, Layer } = newDraw(
        ni.config.drawConfig.type,
        ni.config.sourceLayer,
        ni.config.Layer
      );
