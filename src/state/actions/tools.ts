...
export const toggleTool =
  (ToolId: string, forceOpen: boolean, forceClose: boolean) =>
...
export const dragTool =
  (ToolId: string, position: ControlPosition) => async (dispatch: Dispatch) => {
    const mapId = API.map.getFocusedMapUUID();
    dispatch<DragToolAction>({
      type: types.DRAG_TOOL,
      payload: { ToolId, mapId, position },
    });
  };

export const toggleToolByName =
  (ToolName: string, forceOpen: boolean, forceClose: boolean) =>
...
export const toggleGroupTool =
  (GroupToolId: string) => (dispatch: Dispatch) => {
    const mapId = API.map.getFocusedMapUUID();
...
export const setToolPosition =
  (ToolId: string, position: ControlPosition) => (dispatch: Dispatch) => {
    const mapId = API.map.getFocusedMapUUID();
    dispatch<SetToolPosition>({
      type: types.SET_TOOL_POSITION,
      payload: { ToolId, mapId, position },
    });
  };
