...
class MyCustomWidget extends Component {
  ...
  onCoordinateInput = (e, input) => {
    if (!this.checkInput(input)) {
      this.setState({
        goTo: {
          ...this.state.goTo,
          [input.name]: { value: null, showError: true },
        },
      });
    } else {
      this.setState(
        {
          goTo: {
            ...this.state.goTo,
            [input.name]: { value: parseFloat(input.value), showError: false },
          },
        },
        this.checkXY
      );
  ...
  getOlPoint = () => {
    ...
    if (X.value && Y.value) {
      const coord = convertCoordToIsraelTM(this.state.projection, [
        Y.value,
        X.value,
      ]);
      console.log(coord);
                ...
                <div onMouseDownCapture={(e) => e.stopPropagation()}>
                  <Form>
                    ...
                    <Form.Field
                      ...
                      onChange={this.onCoordinateInput}
                      error={
                        this.state.goTo.Y.showError && {
                          content: this.state.goTo.errorMessage,
                        }
                      }
                    />
                    {this.state.goTo.X.value &&
                      this.state.goTo.Y.value &&
                      !this.state.goTo.isValid && (
                        <p>
                          Coordinates are valid numbers but won't fall inside
                          Israel Grid, pleace enter a valid coordinate
                        </p>
                      )}
                    <IconButton
