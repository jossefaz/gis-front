...
function LB() {
  ...
  this.attach = function (endpoint) {
    ...
    if (!isActive) {
      ...
      if (that.writeActivated != null) that.writeActivated();
    }
  };
...
JSMQ.Subscriber = function () {
  ...
  that.subscribe = function (subscription) {
    if (subscription instanceof Uint8Array) {
      ...
      if (JSMQ.useBase64) sub = Base64.encode(sub);
...
JSMQ.Message = function () {
  ...
  this.prependString = function (str) {
    str = String(str);
    if (JSM.useBase64) str = Base64.encode(str);
  ...
  this.addString = function (str) {
    str = String(str);
    if (JSMQ.useBase64) str = Base64.encode(str);
  ...
  this.popString = function () {
    ...
    else return StringUtility.Uint8ArrayToString(frame);
  };
